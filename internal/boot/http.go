...
import (
	...
	// "bisnis-be/internal/data/auth"

	// "bisnis-be/pkg/firebaseclient"

	"bisnis-be/pkg/tracing"
	"log"
	"net/http"
	...
	goldgymData "bisnis-be/internal/data/bisnis"
	goldgymServer "bisnis-be/internal/delivery/http"
	goldgymHandler "bisnis-be/internal/delivery/http/bisnis"
	goldgymService "bisnis-be/internal/service/bisnis"
	...
	// pushNotifService "bisnis-be/internal/service/pushnotif"
)
...
func HTTP() error {
	...
	zapLogger := logger.With(zap.String("service", "bisnis"))
	zlogger := jaegerLog.NewFactory(zapLogger)
	// loggers := Log.NewLogrusLogger()
	// Set tracer for service
	tracer, closer := tracing.Init("bisnis", zlogger)
	defer closer.Close()
