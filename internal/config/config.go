...
func getDefaultConfigFile() string {
	configPath := "./files/etc/bisnis-be/bisnis-be.development.yaml"
	...
	if env != "" {
		...
		case envProduction:
			...
			configPath = "/vault/secrets/database.yaml"
		default:
			configPath = "./bisnis-be.development.yaml"
		}
	}

	if os.Getenv("chc") == "sementara" {
		configPath = "./bisnis-be.chc.production.yaml"
	}
...
// 	configPath := "./files/etc/bisnis-be/credentials.development.json"
// 	namespace, _ := ioutil.ReadFile("/var/run/secrets/kubernetes.io/serviceaccount/namespace")
